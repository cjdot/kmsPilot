...
var qry13 = 'SELECT DISTINCT CONCAT(firstName, \' \', lastName) AS name, users.userID FROM users  LEFT JOIN (SELECT DISTINCT CONCAT(firstName, \' \', lastName) AS name, users.userID, projectID FROM users LEFT JOIN projectassignment ON users.userID = projectassignment.userID WHERE projectID = @projectID) AS sub ON CONCAT(users.firstName, \' \', users.lastName) = sub.name WHERE sub.name IS NULL; '
