...
router.post('/', ensureAuthenticated, function(req, res){
	...
	if (dateRange == 'thisWeek'){
		...
		var qry1 = 'SELECT TOP 10 kmsactionitem.projectID, project.projectName, kmsactionitem.actionItemDescription, kmsactionitem.targetCompletionDate FROM kmsactionitem INNER JOIN project ON kmsactionitem.projectID = project.projectID INNER JOIN users ON kmsactionitem.activityOwner = CONCAT(users.firstName, \' \', users.lastName) WHERE project.openProject = \'open\' AND users.email = @email AND kmsactionitem.targetCompletionDate IS NOT NULL AND kmsactionitem.targetCompletionDate >= GETDATE() AND kmsactionitem.targetCompletionDate <= DATEADD(day, 7, GETDATE()) ORDER BY kmsactionitem.targetCompletionDate DESC';
		var qry2 = 'SELECT DISTINCT TOP 10 externalactionitem.projectID, project.projectName, externalactionitem.actionItemDescription, externalactionitem.targetCompletionDate, users.email FROM project INNER JOIN externalactionitem ON project.projectID = externalactionitem.projectID INNER JOIN projectassignment ON project.projectID = projectassignment.projectID INNER JOIN users ON projectassignment.userID = users.userID WHERE project.openProject = \'open\' AND users.email = @email AND externalactionitem.targetCompletionDate IS NOT NULL AND externalactionitem.targetCompletionDate >= GETDATE() AND externalactionitem.targetCompletionDate <= DATEADD(day, 7, GETDATE()) ORDER BY externalactionitem.targetCompletionDate DESC';
		var qry3 = 'SELECT (COUNT(kmsactionitem.targetCompletionDate) - COUNT(kmsactionitem.actualCompletionDate)) AS totalTarget, COUNT(kmsactionitem.actualCompletionDate) AS totalActual FROM kmsactionitem INNER JOIN project ON kmsactionitem.projectID = project.projectID WHERE project.openProject = \'open\''
		var qry4 = 'SELECT (COUNT(externalactionitem.targetCompletionDate) - COUNT(externalactionitem.actualCompletionDate)) AS totalTarget2, COUNT(externalactionitem.actualCompletionDate) AS totalActual2 FROM externalactionitem  INNER JOIN project ON externalactionitem.projectID = project.projectID WHERE project.openProject = \'open\''
		...
		var qry1 = 'SELECT TOP 10 kmsactionitem.projectID, project.projectName, kmsactionitem.actionItemDescription, kmsactionitem.targetCompletionDate FROM kmsactionitem INNER JOIN project ON kmsactionitem.projectID = project.projectID INNER JOIN users ON kmsactionitem.activityOwner = CONCAT(users.firstname, \' \', users.lastName) WHERE users.email = @email AND project.openProject = \'open\' AND kmsactionitem.targetCompletionDate IS NOT NULL AND kmsactionitem.targetCompletionDate >= GETDATE() AND kmsactionitem.targetCompletionDate <= DATEADD(day, 14, GETDATE())  ORDER BY kmsactionitem.targetCompletionDate DESC';
		...
		var qry1 = 'SELECT TOP 10 kmsactionitem.projectID, project.projectName, kmsactionitem.actionItemDescription, kmsactionitem.targetCompletionDate FROM kmsactionitem INNER JOIN project ON kmsactionitem.projectID = project.projectID INNER JOIN users ON kmsactionitem.activityOwner = CONCAT(users.firstName, \' \', users.lastName) WHERE users.email = @email AND project.openProject = \'open\' AND kmsactionitem.targetCompletionDate IS NOT NULL AND kmsactionitem.targetCompletionDate >= GETDATE() AND kmsactionitem.targetCompletionDate <= DATEADD(day, 30, GETDATE())  ORDER BY kmsactionitem.targetCompletionDate DESC';
		...
		var qry1 = 'SELECT TOP 10 kmsactionitem.projectID, project.projectName, kmsactionitem.actionItemDescription, kmsactionitem.targetCompletionDate FROM kmsactionitem INNER JOIN project ON kmsactionitem.projectID = project.projectID INNER JOIN users ON kmsactionitem.activityOwner = CONCAT(users.firstName, \' \', users.lastName) WHERE users.email = @email AND project.openProject = \'open\' AND kmsactionitem.targetCompletionDate IS NOT NULL AND kmsactionitem.targetCompletionDate <= GETDATE() ORDER BY kmsactionitem.targetCompletionDate DESC';
		var qry2 = 'SELECT DISTINCT TOP 10 externalactionitem.projectID, project.projectName, externalactionitem.actionItemDescription, externalactionitem.targetCompletionDate FROM project INNER JOIN externalactionitem ON project.projectID = externalactionitem.projectID INNER JOIN projectassignment ON project.projectID = projectassignment.projectID INNER JOIN users ON projectassignment.userID = users.userID WHERE project.openProject = \'open\' AND externalactionitem.targetCompletionDate IS NOT NULL AND externalactionitem.targetCompletionDate <= GETDATE() AND users.email = @email ORDER BY externalactionitem.targetCompletionDate DESC';
		var qry3 = 'SELECT (COUNT(kmsactionitem.targetCompletionDate) - COUNT(kmsactionitem.actualCompletionDate)) AS totalTarget, COUNT(kmsactionitem.actualCompletionDate) AS totalActual FROM kmsactionitem INNER JOIN project ON kmsactionitem.projectID = project.projectID WHERE project.openProject = \'open\''
		var qry4 = 'SELECT (COUNT(externalactionitem.targetCompletionDate) - COUNT(externalactionitem.actualCompletionDate)) AS totalTarget2, COUNT(externalactionitem.actualCompletionDate) AS totalActual2 FROM externalactionitem  INNER JOIN project ON externalactionitem.projectID = project.projectID WHERE project.openProject = \'open\''
	...
	var qry1 = 'SELECT TOP 10 kmsactionitem.projectID, project.projectName, kmsactionitem.actionItemDescription, kmsactionitem.targetCompletionDate FROM kmsactionitem INNER JOIN project ON kmsactionitem.projectID = project.projectID INNER JOIN users ON kmsactionitem.activityOwner = CONCAT(users.firstName, \' \', users.lastName) WHERE users.email = @email AND project.openProject = \'open\' AND kmsactionitem.targetCompletionDate IS NOT NULL AND kmsactionitem.targetCompletionDate >= GETDATE() AND kmsactionitem.targetCompletionDate <= DATEADD(day, 7, GETDATE())  ORDER BY kmsactionitem.targetCompletionDate DESC';
